...
mc = None
BASE_URL="http://www.uread.com/search-books/{0}"
...
class UreadParser:
    ...
    def get_page(self,search_term,search_type="Rest"):
        val=self.mc.get(getkey("%s%s%s" % ("uread",search_term,search_type)))
        if val is None:
            ...
            self.mc.set(getkey("%s%s%s" % ("uread",search_term,search_type)),tostring(val))
        return val
    ...
    def parse(self,search_term,search_type):
        ...
        price_d = d('div#listSearchResult.product div.list-view-books div.product-summary div.price-attrib div.price div.sell').remove('span').map(lambda i, e: pq(e).text())
        for p in price_d:
            logger.debug(p)

        name_d = d('div#listSearchResult.product div.list-view-books div.product-summary div.title a').map(lambda i, e: pq(e).text())
        for p in name_d:
            logger.debug(p)
        ...
        img_d = d('div#listSearchResult.product div.list-view-books div.cover a img').map(lambda i, e: pq(e).attr('src'))
        for p in img_d:
            logger.debug(p)

        url_d = d('div#listSearchResult.product div.list-view-books div.product-summary div.title a').map(lambda i, e: pq(e).attr('href'))
        for p in url_d:
            logger.debug(p)
        ...
        for price,name,author,discount,img,url in map(None, price_d,name_d,author_d,discount_d,img_d, url_d):
            if price:
                ...
                price={'uuid':uuid_tmp,'source':'http://localhost/static/cache/images/stores/Uread.png', 'price':float(sanitize_price(price)),
                       'name':titlecase(name),
                       'author':author,
